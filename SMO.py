...
class SMO:
    ...
    def train(self, tol, max_iters=100, ident=0, load=False):
        if load:
            self.alpha = np.loadtxt("alpha_" + str(ident) + ".csv").reshape(self.features.shape[0], 1)
            ...
            while iters < max_iters and (changed_alphas > 0 or examine_all):
                ...
                if examine_all:
                    ...
                    examine_all = True
                iters += 1
                logging.info(f"iter: {iters}, b: {self.b}, changed alphas: {changed_alphas},"
                             f" unbound: {len(self.unbound_alphas)}, bound : {len(self.bound_alphas)}")
                # print(f"iter: {iters}, b: {self.b}, changed alphas: {changed_alphas},
                # unbound: {len(self.unbound_alphas)}, bound : {len(self.bound_alphas)}")
    ...
    def train_accuracy(self):
        ...
        for i in range(self.labels.shape[0]):
            prediction = self.train_hypothesis(i)
            if prediction >= 0 and self.labels[i] >= 0 or prediction < 0 and self.labels[i] < 0:
                count += 1
        return count/len(self.labels)

    def visualize(self, ident=0):
        t_size = int(np.sqrt(self.features.shape[1]))
        p = (np.transpose(self.features).dot(self.alpha)).reshape(t_size, t_size)
        plt.imshow(p, cmap='hot', interpolation='nearest')
        plt.title(f"SVM for  {ident}")
        plt.show()
